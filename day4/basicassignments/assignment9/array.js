...
function find(){
    const input = document.getElementById('numberToFind').value.trim();
    if (input === '') {
    ...
    if (index === -1) {
        alert(`The number ${numberToFind} is not found in the array.`);
    }
    document.getElementById('findindex').innerText = `Index of ${numberToFind}: ${index}`;
    document.getElementById('numberToFind').value = '';
}
